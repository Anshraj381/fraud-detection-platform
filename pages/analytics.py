...
from frontend.utils import get_awareness_level_name, inject_custom_css, create_score_card
    ...
    with col3:
        ...
        st.markdown(
            create_score_card(
                "Awareness Level",
                level_name,
                f"Score: {awareness_score:.1f}/100",
                "green"
            ),
            unsafe_allow_html=True
        )
...
def display_risk_distribution(analytics: dict):
    ...
    with col2:
        st.warning(f"Suspicious: {risk_dist.get('Suspicious', 0)}")
    with col3:
...
def display_history_controls():
    ...
    st.warning(
        "**Warning:** Clearing history will permanently delete all analysis records. "
